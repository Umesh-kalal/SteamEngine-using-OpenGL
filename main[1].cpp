...
/* Variable used in the creation of glut objects */
GLUquadricObj *obj;
...
/* Draws a cylinder using glut function, drawing flat disc's at each end,
   to give the appearance of it being solid. */
void
...
/* Called when a key is pressed. Checks if it recognize the key and if so
   acts on it, updating the screen. */
/* ARGSUSED1 */
void
...
/* Called when a menu option has been selected. Translates the menu item
   identifier into a keystroke, then calls the keyboard function. */
void
menu(int val)
{
  unsigned char key;
...
/* Makes a simple check pattern image. (Copied from the red book example
   "checker.c".) */
void
make_image(void)
{
...
/* Initialize texturing, lighting, display lists, and everything else
...
void display_other()
{
    ...
    std::string text8;
    std::string text9;
    text1 = "Steam Engine by umesh abbanna and sachin chavan";
...
/* Main program. An interactive model of a miniature steam engine.
   Sets system in Double Buffered mode and initializes all the call-back
   functions. */
int
main(int argc, char **argv)
{
  ...
  */
  ...
  /* Transparency won't work properly without GLUT_ALPHA */
